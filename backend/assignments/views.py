...
# filters out dotfiles and __MACOSX entries that pollute zip archives
...
# strips quotes and non-ascii to prevent header injection in content-disposition
...
class AssignmentDetailView(APIView):
    ...
    @extend_schema(
        ...
        description=(
            "Partially update an assignment's title, "
            "description, prompt, source_text, or rubric."
        ),
        request=AssignmentSerializer,
        responses={
            200: AssignmentSerializer,
...
class AssignmentUploadView(APIView):
    ...
    def post(self, request: AuthenticatedRequest, assignment_id: uuid.UUID) -> Response:
        ...
        result = process_essay_batch.delay(essay_ids)
    ...
    def _handle_zip(
        ...
        # silently returns None on corrupt zips so caller can show a user-facing error
        try:
...
class AssignmentExportCSVView(APIView):
    ...
    def get(
        ...
        for criterion in criteria:
            header.append(f"{criterion.name} (score)")
            header.append(f"{criterion.name} (feedback)")
        header.append("total_score")

        writer = csv.writer(response)
        writer.writerow(header)

        essays = assignment.essays.prefetch_related(
            "grading_result__criterion_scores__level",
            "grading_result__criterion_scores__teacher_level",
            "grading_result__criterion_scores__criterion",
        ).all()

        for essay in essays:
            row: list[str] = [essay.file_name, essay.status]
            total_score = 0
            has_scores = False

            grading_result = getattr(essay, "grading_result", None)

            scores_by_criterion: dict[str, object] = {}
            if grading_result:
                for cs in grading_result.criterion_scores.all():
                    scores_by_criterion[str(cs.criterion_id)] = cs

            for criterion in criteria:
                cs = scores_by_criterion.get(str(criterion.id))
                if cs:
                    has_scores = True
                    # uses teacher_level if overridden, otherwise falls back to ai_level
                    effective_level = cs.teacher_level or cs.level
                    effective_feedback = cs.teacher_feedback or cs.feedback
                    row.append(str(effective_level.score))
                    row.append(effective_feedback)
                    total_score += effective_level.score
                else:
                    row.append("")
                    row.append("")

            row.append(str(total_score) if has_scores else "")
            writer.writerow(row)
